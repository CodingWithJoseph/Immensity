...
export async function POST(request: NextRequest, { params }: { params: Promise<{ pipelineId: string }> }) {
    ...
    const res = await fetch(`${API_URL}/portfolio/${pipelineId}/usage-source`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
    })
    return forwardJson(res, 'Failed to enable usage monitor')
}
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ pipelineId: string }> }) {
    ...
    return forwardJson(res, 'Failed to save usage setup')
}
