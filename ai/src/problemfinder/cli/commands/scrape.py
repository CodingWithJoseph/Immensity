...
def handle_scrape(
    ...
    total_rows = 0
    supplied_factories = {
        "github-issues": github_scraper_factory,
        "github-discussions": github_discussions_scraper_factory,
        "reddit": reddit_scraper_factory,
        "stackexchange": stackexchange_scraper_factory,
        "hackernews": hackernews_scraper_factory,
    }
    dependency_injection_mode = any(factory is not None for factory in supplied_factories.values())

    def should_run(name: str, selected: set[str]) -> bool:
        if args.source not in selected:
            return False
        return not dependency_injection_mode or supplied_factories[name] is not None

    if should_run("github-issues", {"all", "github", "github-issues"}):
        ...
        rows, report = github.scrape(repositories, lookback_days=args.github_lookback_days, limit_per_repository=args.github_limit_per_repository, max_total=args.github_max_items)
        rows_written = int(github.save_to_db(rows, connect_fn=connect_fn).get("rows", 0))
        results["github_issues"] = {**report, "rows_written": rows_written}
        total_rows += rows_written

    if should_run("github-discussions", {"all", "github", "github-discussions"}):
        ...
        rows, report = discussions.scrape(repositories, limit_per_repository=args.github_discussion_limit_per_repository, max_total=args.github_discussion_max_items)
        rows_written = int(discussions.save_to_db(rows, connect_fn=connect_fn).get("rows", 0))
        results["github_discussions"] = {**report, "rows_written": rows_written}
        total_rows += rows_written

    if should_run("reddit", {"all", "reddit"}):
        ...
        posts = reddit.scrape(subreddits, limit_per_feed=args.limit_per_feed, feeds=args.reddit_feed, max_posts=args.reddit_max_posts)
        rows_written = int(reddit.save_to_db(posts, connect_fn=connect_fn).get("rows", 0))
        results["reddit"] = {"subreddits_targeted": len(subreddits), "posts_scraped": len(posts), "rows_written": rows_written}
        total_rows += rows_written

    if should_run("stackexchange", {"all", "stackexchange"}):
        ...
        rows, report = stackexchange.scrape(sites, lookback_days=args.stackexchange_lookback_days, limit_per_site=args.stackexchange_limit_per_site, max_total=args.stackexchange_max_items)
        rows_written = int(stackexchange.save_to_db(rows, connect_fn=connect_fn).get("rows", 0))
        results["stackexchange"] = {**report, "rows_written": rows_written}
        total_rows += rows_written

    if should_run("hackernews", {"all", "hackernews"}):
        ...
        hackernews = hackernews_scraper_factory()
        rows, report = hackernews.scrape(feeds=args.hackernews_feed, limit_per_feed=args.hackernews_limit_per_feed, max_total=args.hackernews_max_items, min_comments=args.hackernews_min_comments)
        rows_written = int(hackernews.save_to_db(rows, connect_fn=connect_fn).get("rows", 0))
