...
def parse_repositories(value: str | Iterable[str] | None) -> list[str]:
    """Parse, validate, and de-duplicate owner/repository targets."""
    ...
    for raw in candidates:
        ...
        if not _REPOSITORY_PATTERN.fullmatch(repository):
            raise ValueError(
                f"Invalid GitHub repository {repository!r}; expected owner/repository"
            )
...
class GitHubApiError(RuntimeError):
    """A repository request failed without stopping the other targets."""
    ...
    def _get_json(self, path: str, parameters: dict[str, object]) -> Any:
        ...
        try:
            ...
            raise GitHubApiError(
                f"GitHub API returned HTTP {error.code} for {path}{suffix}"
            ) from error
        ...
        *,
        ...
        for page in range(1, max_pages + 1):
            ...
            if not isinstance(response, list):
                raise GitHubApiError(
                    f"GitHub API returned an unexpected response for {repository}"
                )
    ...
    def __init__(
        ...
        *,
        ...
        self.client = client or GitHubApiClient(
            ...
            base_url=os.getenv("GITHUB_API_URL") or "https://api.github.com",
        )
        ...
        for repository in targets:
            try:
                issues = self.client.list_issues(
                    repository,
                    since=since,
                    max_results=limit_per_repository,
                )
                rows.extend(
                    normalize_github_issue(issue, repository) for issue in issues
                )
                succeeded += 1
